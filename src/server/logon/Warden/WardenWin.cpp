...
void WardenWin::Init(ClientSession* session, BigNumber *k)
{
    _session = session;
    // Generate Warden Key
...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    _previousTimestamp = getMSTime(); // @emo Logon::GetGameTimeMS();
}
...
void WardenWin::RequestData()
{
    ...
    _serverTicks = getMSTime(); // @emo Logon::GetGameTimeMS();
...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    // TIMING_CHECK
    {
        ...
        uint32 ticksNow = getMSTime(); // @emo Logon::GetGameTimeMS();
    ...
    uint32 holdOff = 1; // sLogon->getIntConfig(CONFIG_WARDEN_CLIENT_CHECK_HOLDOFF);
