...
void Warden::Update()
{
    if (_initialized)
    {
        uint32 currentTimestamp = getMSTime(); // @emo Logon::GetGameTimeMS();
        uint32 diff = getMSTimeDiff(_previousTimestamp, currentTimestamp);
        _previousTimestamp = currentTimestamp;

        if (_dataSent)
        {
            uint32 maxClientResponseDelay = 60; /* 60s for response are more than enough */ // sLogon->getIntConfig(CONFIG_WARDEN_CLIENT_RESPONSE_DELAY);
            if (maxClientResponseDelay > 0)
            {
                if (_clientResponseTimer > maxClientResponseDelay * IN_MILLISECONDS)
                {
                    sLog->outString("[WARDEN ANTICHEAT] Player %s got kicked for being idle for more than %u seconds!", _session->GetPlayer()->GetName(), maxClientResponseDelay);
                    _session->KickPlayer();
                }
                else
                {
...
std::string Warden::Penalty(WardenCheck* check /*= NULL*/, uint16 checkFailed /*= 0*/)
{
    ...
    if (check)
        action = check->Action; // @emo
    else
        action = WardenActions(WARDEN_ACTION_KICK); // sLogon->getIntConfig(CONFIG_WARDEN_CLIENT_FAIL_ACTION));
    ...
    case WARDEN_ACTION_LOG:
        sLog->outString("[WARDEN ANTICHEAT][HACKING] Player %s got caught hacking\"%s\", please check him!", _session->GetPlayer()->GetName(), banReason.c_str());
        return "None";
        break;
    case WARDEN_ACTION_KICK:
        if (checkFailed)
            sLog->outString("[WARDEN ANTICHEAT][HACKING] Player %s got kicked for \"%s\"", _session->GetPlayer()->GetName(), banReason.c_str());
        else
            sLog->outString("[WARDEN ANTICHEAT][HACKING] Player %s got kicked. No reason, something custom?", _session->GetPlayer()->GetName());
    ...
    case WARDEN_ACTION_BAN:
        {
            ...
            duration << sLogon->getIntConfig(CONFIG_WARDEN_CLIENT_BAN_DURATION) << "s";
            std::string accountName;
            AccountMgr::GetName(_session->GetAccountId(), accountName);
            //sLogon->BanAccount(BAN_ACCOUNT, accountName, ((longBan && false /*ZOMG!*/) ? "1209600s" : duration.str()), banReason, "Server");
            //sLog->outString("[WARDEN ANTICHEAT][HACKING] Player ACCOUNT %s got banned for \"%s\". Duration: %s", accountName, banReason, ((longBan && false /*ZOMG!*/) ? "1209600s" : duration));
            return "Ban";
        }
    default:
        break;
